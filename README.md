...
The left sidebar provides primary module navigation. The **Research Console** module accepts a company name, alias, or ticker plus risk theme and fiscal years. It resolves the input to SEC ticker, CIK, and standard company name, then retrieves structured SEC `companyfacts` data for deterministic metric extraction. Demo presets are shortcuts only; users can type any SEC EDGAR-covered public company, subject to available companyfacts and configured XBRL concept coverage. This mode does not call Bedrock by default, but it does require network access to `sec.gov`.
